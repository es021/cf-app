...
export function getActivityQueryAttr(type) {
    ...
    switch (type) {
        ...
        case ActivityType.PRESCREEN:
            select = ` ${isRoleRec() ? "pic" : ""} ID company_id is_expired is_onsite_call join_url start_url appointment_time updated_at special_type status ${getEntitySelect(role, type)} `;
            break;
