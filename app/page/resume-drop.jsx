...
export default class ResumeDropPage extends React.Component {
    ...
    loadData() {
        ...
        // load document
        var query = `query{user(ID:${getAuthUser().ID}){
             doc_links{ID label url} }}`;
        getAxiosGraphQLQuery(query).then((res) => {
            data.doc_links = res.data.data.user.doc_links;
            finishLoad();
        });

        // load existing resume drop
        var query = `query{resume_drop(student_id:${getAuthUser().ID}, company_id:${this.company_id}){
    ...
    render() {
        ...
        if (this.state.loading) {
            ...
            // dont have document
            if (this.state.data.doc_links === null) {
                v = <div>
                It seems that you don't have any<br></br>resume uploaded in your profile yet.
                <br></br><br></br>
                <NavLink className="btn btn-primary" to={`${RootPath}/app/edit-profile/doc-link`}>Upload Resume Now</NavLink></div>;
