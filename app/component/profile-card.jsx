...
export const getImageObj = function (obj) {
  if (obj != null && typeof obj !== "undefined") {
    ...
    r.img_pos =
      typeof obj.img_pos != "undefined" ? obj.img_pos : obj.img_position;
    return r;
  } else {
...
export const getStyleBannerObj = function (url, size, pos, width, height) {
  ...
  let styleBanner = getStyleImageObj(
    PCType.BANNER,
    url,
    size,
    pos,
...
export default class ProfileCard extends React.Component {
  ...
  render() {
    ...
    var dimension = this.props.img_dimension
      ? this.props.img_dimension
      : "100px";

    var stylePicture = getStyleImageObj(
      this.props.type,
      this.props.img_url,
      this.props.img_size,
      this.props.img_pos,
    ...
    if (this.props.badge != null) {
      badge = (
        <div className={`${pc}badge ${this.props.badge == "" ? "small" : ""}`}>
          {this.props.badge_tooltip ? (
            <span>
              <div className={`${pc}badge-tooltip-arrow`} />
              <div className={`${pc}badge-tooltip`}>
                {this.props.badge_tooltip}
              </div>
            </span>
          ) : null}
    ...
    if (this.props.add_img_ops) {
      img_ops = (
        <div className={`${pc}picture-ops`}>
          <ButtonIcon
            icon="edit"
            theme="dark"
            onClick={() => this.openPictureOps(stylePicture)}
          />
        </div>
      );
    }
    ...
    if (this.props.addBanner) {
      let styleBanner = getStyleBannerObj(
        this.props.banner_url,
        this.props.banner_size,
        this.props.banner_pos,
        this.props.custom_width,
        this.props.banner_height
      );
