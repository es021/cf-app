...
class SocketServer {
  ...
  initOn(client) {
    client.on(C2S.JOIN, d => {
      this.onJoin(client, d);
    });
    client.on(C2S.LOGOUT, d => {
      this.onLogout(client, d);
    });

    client.on(C2S.DISCONNECT, d => {
      this.onDisconnect(client, d);
    });
    client.on(BOTH.CHAT_OPEN_CLOSE, d => {
      this.onChatOpenClose(client, d);
    });
    client.on(BOTH.CHAT_MESSAGE, d => {
      this.onChatMessage(client, d);
    });
    client.on(BOTH.LIVE_FEED, d => {
      this.onLiveFeed(client, d);
    });
    client.on(BOTH.STATE, d => {
      this.onState(client, d);
    });
    client.on(BOTH.HALL_ACTIVITY, d => {
      this.onHallActivity(client, d);
    });
    client.on(BOTH.PROGRESS, d => {
      this.onProgress(client, d);
    });
  ...
  emitToRole(emit, data, role, cf) {
    for (var i in this.state.clients) {
      if (
        this.state.clients[i].role === role &&
        this.state.clients[i].cf === cf
      ) {
        this.emitToClient(this.state.clients[i], emit, data);
      }
    }
  ...
  emitToClient(client, emit, data) {
    ...
    for (var i in client.sockets) {
      ...
      this.printHeader(
        "[emit -> " + emit + "] To (" + client.id + ") : " + soc.socket.id
      );
  ...
  updateEmitOnlineCompany(data, event, client) {
    ...
    if (!client) {
      ...
      this.emitToClient(client, S2C.ONLINE_COMPANY, this.state.online_company);
    }
  }
